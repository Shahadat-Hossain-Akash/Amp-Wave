...
export const getOrders = async (req,res)=> {
    ...
    const apiFilter = new APIFilters(Order.find(), req.query).pagination(resPerPage)

    const orders = await apiFilter.query.find().populate("shippingInfo user")

    res.status(200).json({
        orders,resPerPage,orderCount
    })
}
...
export const myOrders = async (req,res)=> {
    ...
    const orders = await apiFilter.query.find({user: req.user._id}).populate("shippingInfo user")

    console.log(orders,resPerPage,orderCount)
...
export const updateOrder = async (req,res)=> {
    ...
    order = await Order.findByIdAndUpdate(req.query.id, {orderStatus: req.body.orderStatus})
...
export const checkoutSession = async (req, res) => {
    ...
    const line_items = body?.items?.map((item)=>{
        return {
            ...
            tax_rates:['txr_1NdI69SH9bGPvgDqpTOYnVRa'],
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        shipping_options: [
            {
                shipping_rate: 'shr_1NdHxsSH9bGPvgDqPfyChyBD'
